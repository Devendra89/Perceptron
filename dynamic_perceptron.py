...
def dynamic_perceptron(rows, Y, learn_rate, w, updates,time_step):
    for i, x in enumerate(rows):
        if Y[i] * np.sum(w * np.array(x)) <=0 :
            w += ((learn_rate/1+time_step)*Y[i])*np.array(x)
            updates +=1
    return updates, w
...
def cross_validation(cv_sets, learn_rate, weight):
    for i in range(len(cv_sets)):
        ...
        for i in range(0, 10):
            ...
            random.seed(i*10)
            random.shuffle(cv_Y)
            updates, w = dynamic_perceptron(cv_X, cv_Y, learn_rate, w,updates, time_step)
            time_step +=1
...
def perceptron_main(train_set, dev_set, test_set,sets, rates):
    ...
    for i in range(0, 20):
        ...
        random.seed(i*123)
        random.shuffle(Y)
        count_updates, w = dynamic_perceptron(X, Y, learn_rate, initial_w, count_updates,time_step) 
        time_step +=1
...
if __name__ == "__main__":
    ...
    dev_set = _dev_set
    test_set = _test_set
    print("running for dynamic Perceptron")
