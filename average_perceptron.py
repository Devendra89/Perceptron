...
def perceptron(rows, Y, learn_rate, w, updates,a):
    average_weight = a
    for i, x in enumerate(rows):
        if Y[i] * np.sum(w * np.array(x)) <=0 :
            w += (learn_rate*Y[i])*np.array(x)
            updates +=1
        average_weight = average_weight + w
    return updates, w,average_weight
...
def cross_validation(cv_sets, learn_rate, weight,avg_weight):
    for i in range(len(cv_sets)):
        ...
        for i in range(0, 10):
            ...
            random.seed(i*10)
            random.shuffle(cv_Y)
            updates, w, a= perceptron(cv_X, cv_Y, learn_rate, w, updates,a)
        cv_x_test, cv_y_test_act = create_data(cv_test_set) 
        cv_y_test = predict(a, cv_x_test)
...
def best_learn_rate(sets, rates, w,a):
    best_rate = {}
    for rate in rates:
        best_rate[rate] = cross_validation(sets, rate, w,a)
...
def perceptron_main(train_set, dev_set, test_set,sets, rates):
    ...
    a = np.array([0.001] * 69)
    learn_rate = best_learn_rate(sets, rates, initial_w,a)
    ...
    for i in range(0, 20):
        ...
        random.seed(i*123)
        random.shuffle(Y)
        count_updates, initial_w,a = perceptron(X, Y, learn_rate, initial_w, count_updates,a) 
        pred_dev_y = predict(a, X_dev)
...
if __name__ == "__main__":
    ...
    print("running for average perceptron")
